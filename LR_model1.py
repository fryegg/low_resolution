...
class HighResolution(nn.Module):
    def __init__(self):
        ...
        self.model_right = resnet()
        self.model_middle = resnet()

    def forward(self, x):
        right_x = self.model_right(x)
        middle_x = self.model_middle(x)
        return right_x + middle_x


class LowResolution(nn.Module):
    def __init__(self, middle_layer):
        super(LowResolution, self).__init__()
        self.model_middle = middle_layer
        self.model_left = resnet()

    def forward(self, x):
        middle_x = self.model_middle(x)
        left_x = self.model_left(x)
        return middle_x + left_x
